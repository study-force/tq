...
5. 각 슬롯은 반드시 2~3문단, 300~500자 범위로 작성 (공백 포함 기준). 이 범위를 벗어나면 안 됨 — 너무 짧거나 너무 긴 답변 모두 금지
...
## 슬롯별 작성 기준
슬롯①: 강점 먼저 → 현실 대비 → 가능성 (300~500자, 2~3문단)
슬롯②: causal_paths 인과 서사 + 체크리스트 기반 현상 해석 포함 (300~500자, 2~3문단)
슬롯③: 긴급도 비례 경각심 + 학제 전환 리스크 (300~500자, 2~3문단)
슬롯④: 처방 우선순위 서술 + 솔루션 연결 + 희망 마무리 (300~500자, 2~3문단)
...
출력 규칙:
- 반드시 유효한 JSON만 출력
- 각 슬롯 본문은 반드시 2~3개 단락으로 구성하고, 단락 사이는 "\\n\\n"(이스케이프된 빈 줄)으로 구분할 것
- 한 단락 안에서는 문장 사이 줄바꿈 금지 (자연스러운 wrap으로만 처리)
- 각 슬롯의 전체 길이는 공백 포함 300~500자 범위를 반드시 지킬 것
- 큰따옴표 포함 시 \\" 이스케이프
...
【필수1】 각 슬롯 300~500자(공백 포함), 체크리스트 항목을 슬롯②에 반드시 반영하세요.
