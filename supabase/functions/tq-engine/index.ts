...
function collectEvidence(inp: Record<string,any>, kv: number|null, flagC: boolean, flagO: boolean, habClass: Record<string,any>) {
  ...
  { const b: any[] = [];
    ...
    if(voc<60&&acc>=70)mpush(b,"3_6","주의","독해정확도에 비해 어휘력이 발달하지 않음 — 익숙한 분야 위주의 독서 편중 가능성 높음");
    ...
    if(voc<60&&acc>=70&&inf<60)mpush(b,"eng_extra_19","주의","독해정확도와 독해습관은 양호하나 어휘·추론이 낮음 — 독서량 부족보다 독서 영역의 편중이 원인으로 추정됨. 익숙한 장르·주제 위주로만 읽어온 것으로 보임");
    if(voc<60&&acc>=70&&inf<60)mpush(b,"eng_extra_20","주의","다양한 비문학 분야의 독해 경험을 넓히는 것이 어휘와 추론능력 향상의 핵심 과제");
  ...
  { const b: any[] = [];
    ...
    if(wm>=20&&wm<=30)mpush(b,"eng_6_5","주의","단순화된 학습 환경과 스토리북·학습만화 위주 독서 경험이 워킹메모리 발달을 저해했을 가능성");
  ...
  { const b: any[] = [];
    ...
    if(자기인식부족){
      if(괴리정도>=4){mpush(b,"eng_extra_23","위험","역량 수준에 비해 체크 항목이 현저히 적음 — 자신의 학습 상태를 심각하게 과대평가하고 있거나 문제를 전혀 인지하지 못하는 상태");mpush(b,"eng_extra_24","위험","자기 인식이 부족한 학습자의 전형적 패턴 — 객관적 역량과 자기 평가 간의 괴리가 클수록 개선 의지를 이끌어내기 어려움");}
      else if(괴리정도>=3){mpush(b,"eng_extra_25","위험","자신의 공부 문제를 충분히 인지하지 못하고 있거나 스스로에게 관대한 편");mpush(b,"eng_extra_26","주의","자기 인식과 실제역량의 괴리가 클수록 개선 의지를 이끌어내기 어려움");}
      else mpush(b,"eng_extra_27","주의","자신에 대한 평가가 관대한 편 — 스스로 문제없다고 여기지만 실제 역량과 괴리가 있는 상태");
    }else if(acc>=70&&acc<80&&총체크수===0&&inp.uiLevel!=="고등"){
      mpush(b,"eng_extra_28","보통","현재는 학습에 큰 어려움을 느끼지 않는 상태");
    }
  ...
  { const b: any[] = [];
    ...
    // eng_extra_29~32 삭제: mathVal 조건으로 국어 메시지를 출력하는 논리 오류. eng_extra_43~45가 kv(국어) 조건으로 동일 역할을 올바르게 수행함.
    if(flagC&&kv!==null&&kv>=90&&acc<50)mpush(b,"eng_extra_43","위험","국어 성적 최상위이나 독해역량이 낮음 — 암기와 스킬에 의존해 성적을 유지하고 있을 가능성이 높으며, 고등 진학 후 한계에 부딪힐 수 있음");
    if(kv!==null&&kv>=90&&acc>=50&&acc<=69)mpush(b,"eng_extra_44","주의","국어 성적 최상위이나 독해역량은 이에 미치지 못함 — 현재 성적은 상당한 노력 투입의 결과로 추정되며, 역량 강화 없이는 유지에 한계가 있음");
    if(kv!==null&&kv>=80&&kv<=90&&acc>=50&&acc<=69)mpush(b,"eng_extra_45","주의","국어 성적 상위권이나 독해역량이 낮음 — 성적 유지를 위해 과도한 노력이 투입되고 있을 가능성, 역량 개선이 근본 해결책");
